...
from pna_metrics import knn_layer
from pna_models import pretty_model_name, get_family_name, get_models
from pna_data import clamp_tensor_outliers, EMB_DIR, build_flikr8k_text_audio_image, build_flikr8k_text_audio_image, save_dataset_index

rng = np.random.default_rng(5)  # For reproducibility

PLOTS_DIR = "../results/sanity_checks/"

def get_data(model_name, modality, chunk_number=0, chunk_size=4000, clip=True):
    ...
    if modality in ["text", "speech"]:
        ...
        mapped_df = mapped_df[:chunk_size]  # Limit to the first chunk_size rows


    if modality == "image":#XXX
        index_df = pd.read_csv(f"{EMB_DIR}/{modality}/dataset_index.csv")
        cunk_size = 800
        mapped_df = index_df.iloc[np.arange(chunk_number * chunk_size, (chunk_number + 1) * chunk_size)].reset_index(drop=True)

    if clip:

        data["avg"] = clamp_tensor_outliers(data["avg"].float(), q=0.9, exact=True)
...
def compute_knn_image(df, index, k):
    ...
    for layer in range(num_layers):
        indices = knn_layer(embeddings[:, layer, :], k=k)
...
def compute_knn_caption(df, index, k, modality="text"):
    ...
    print(f"Columns in index: {index.columns.tolist()}")
    print(f"Lenght of index DataFrame: {len(index)}")
    
    print(len(captions), len(image_ids), embeddings.shape)

    model_chunk_avg = []

    indices = np.zeros((embeddings.shape[0], embeddings.shape[1], k), dtype=int)

    for layer in range(indices.shape[1]):
        indices[:, layer, :] = knn_layer(embeddings[:,layer,:], k=k)
...
def plot_speaker_mean_embeddings(speaker_means, model_name, type="speaker_means"):
    ...
    model_name_p = get_family_name(modality="speech", model_name=model_name) + " " + pretty_model_name(model_name)
    plt.title(
    ...
    plt.savefig(
            f"{PLOTS_DIR}"
            f"pca_speaker_means_{model_name_p}_layer_all_{type}.png"
        )
